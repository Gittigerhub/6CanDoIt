...
public class ReservationController {
    ...
    public ResponseEntity<?> cancelReservation(@PathVariable Integer idx) {
        log.info("일반 사용자 예약 취소 처리 시작.... idx: {}", idx);
        try {
            reservationService.reserveDelete(idx);
            return ResponseEntity.ok().body(Collections.singletonMap("message", "예약이 취소되었습니다."));
        } catch (Exception e) {
    ...
    // 관리자용 예약 수정 처리
    @PostMapping("/ho/update/{idx}")
    @ResponseBody
    public ResponseEntity<?> updateMemberReservation(@PathVariable Integer idx, @RequestBody ReservationDTO reservationDTO) {
